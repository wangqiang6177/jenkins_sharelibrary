...
//        }
        ...
        stage ('代码克隆') {
            steps{
				...
				echo "代码仓库地址:$gitlabSourceRepoHttpUrl"
				echo "$env.gitlabMergeRequestAssignee $env.gitlabUserEmail "
				echo "-----------------------"
...
echo "gitlabTriggerPhrase                $env.gitlabTriggerPhrase               "
        ...
        stage('编译打包'){
            steps{
                echo "Building..."
                sh '''#!/bin/bash
                cd ${WORKSPACE}/${_pomDir} && mvn clean package -DskipTests=true'''
			}
        }
        stage('单元测试'){
            steps{
